...
function Package({number, title, description, subDescription, tourImg, order}) {
  return (
    <div className='flex flex-1 flex-col items-center justify-between md:flex-row my-16 md:gap-10'>
       <div className={` ${order === 'reverse' ? 'md:order-2 md:items-end' : 'md:order-1 items-start'} order-2
       max-w-6xl w-full flex flex-col text-white mt-10 md:mt-0`}>
        ...
        <div className={`${order === 'reverse' ? 'md:order-1' : 'md:order-2'} order-1 h-[400px] w-full bg-cover bg-no-repeat rounded-lg md:w-[400px] md:h-[450px] md:mt-0`} style={{
